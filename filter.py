...
def update_half_year(year, is_first_half):
	"""Updates the from and to date in UTC epoch per half year."""
	if (is_first_half):
		half = first_half
	else:
		half = sec_half
...
def generate_json_per_halfyear(year):
	...
	while(cur_year > 2010):
		update_half_year(cur_year, is_first_half)
...
def check_questions_per_year(year):
	# Validates that number of question posts per year are indeed 100
	while(year >= 2008):
		file_name = 'data_' + str(year) + '.json'
		path = os.path.join("data", file_name)
...
def check_questions_per_halfyear(year):
	# Validates that number of question posts per half a year are indeed 100
	is_first_half = False
	cur_year = year
	time_frame = ''
	...
	while(cur_year > 2010):
		...
		print 'Number of questions for year %s in %d: %d ' % (time_frame, cur_year, 
			len(data['items']))
